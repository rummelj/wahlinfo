...
public class CsvParser implements ICsvParser {
    ...
    @Override
    public Collection<CsvElectoralDistrictVoteData> parseDistrictVoteData() throws CsvParserException {
        CsvReader reader = buildCsvReader(FILE_PATH_VOTES);
        Collection<CsvElectoralDistrictVoteData> res = new HashSet<>();
        try{
            reader.readHeaders();
            while(reader.readRecord()){
                res.add(new CsvElectoralDistrictVoteData(reader.get(0), ElectionYear._2009, reader.get(3), 
                        reader.get(7), reader.get(15), reader.get(17), reader.get(11), reader.get(13)));
                res.add(new CsvElectoralDistrictVoteData(reader.get(0), ElectionYear._2005, reader.get(4), 
                        reader.get(8), reader.get(16), reader.get(18), reader.get(12), reader.get(14)));
            }
     ...
     * prevent the creation of false duplicates (e.g. "DIE LINKE" & "Die
     * Linke."). All entries do not conform to the specification of
