...
public class VoteGenerator implements IVoteGenerator {
	 ...
	 * @param aggregation
	 * @param shadowIdStart
	 *            Required to prevent hash collisions between otherwise
	 *            identical votes.
	 * @return
	 */
	public Collection<CsvGeneratedVote> generateVotes(
			CsvVoteAggregation aggregation, long shadowIdStart) {
		Collection<CsvGeneratedVote> part1 = generateVotes(aggregation,
				shadowIdStart, ElectionYear._2005);
		Collection<CsvGeneratedVote> part2 = generateVotes(aggregation,
				shadowIdStart + part1.size() + 1, ElectionYear._2009);
		part1.addAll(part2);
		return part1;
	}

	private Collection<CsvGeneratedVote> generateVotes(
			CsvVoteAggregation aggregation, long shadowIdStart,
			ElectionYear year) {
		...
		while (true) {
			...
			if (numDirVotes == -1 && numListVotes == -1) {
				...
				votes.add(new CsvGeneratedVote(id++, aggregation
